...
func (s *RedisStore) Acquire(ctx context.Context, scene, key string, ttl time.Duration) (string, bool, error) {
	...
	rec := redisRecord{Scene: scene, Key: key, Token: token, Status: "PROCESSING", UpdatedAt: now.UnixMilli(), ExpireAt: now.Add(ttl).UnixMilli()}
...
func (s *RedisStore) MarkDone(ctx context.Context, scene, key, token string, result []byte) error {
	...
	_, err := markDoneScript.Run(ctx, s.client, []string{s.buildKey(scene, key)}, token, resultB64, time.Now().UnixMilli()).Result()
	return err
}

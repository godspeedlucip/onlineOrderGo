...
const contextKeyTx txContextKey = "product_sql_tx"
...
func (m *SQLManager) BeginTx(ctx context.Context) (context.Context, *sql.Tx, error) {
	...
	if err != nil {
		return ctx, nil, domain.NewBizError(domain.CodeInternal, "begin transaction failed", err)
	}
...
func (m *SQLManager) Commit(tx *sql.Tx) error {
	...
	if err := tx.Commit(); err != nil {
		return domain.NewBizError(domain.CodeInternal, "commit transaction failed", err)
	}
	return nil
}
...
func (m *SQLManager) Rollback(tx *sql.Tx) error {
	...
	if err := tx.Rollback(); err != nil && err != sql.ErrTxDone {
		return domain.NewBizError(domain.CodeInternal, "rollback transaction failed", err)
	}
	return nil
}

...
var markFailedDeleteScript = redis.NewScript(`
local raw = redis.call('GET', KEYS[1])
if not raw then
	return 0
end
...
func (s *RedisStore) MarkDone(ctx context.Context, scene, key, token string, result []byte) error {
	...
	_, err := markDoneScript.Run(ctx, s.client, []string{s.buildKey(scene, key)}, token, encoded, time.Now().UnixMilli()).Result()
	return err
}
...
func (s *RedisStore) MarkFailed(ctx context.Context, scene, key, token, reason string) error {
	...
	_, err := markFailedDeleteScript.Run(ctx, s.client, []string{s.buildKey(scene, key)}, token).Result()
	return err
}
...
func (s *RedisStore) GetDoneResult(ctx context.Context, scene, key string) ([]byte, bool, error) {
	...
	out, err := base64.StdEncoding.DecodeString(rec.ResultB64)
	if err != nil {
		return nil, false, err
	}
	return out, true, nil
